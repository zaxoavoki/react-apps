...
function App() {
    ...
    const onClick = useCallback((task) => setTasks({
        tasks: tasks.tasks.map(e => Object.assign(e, {done: e.id === task.id ? !e.done : e.done})),
        error: tasks.error
    }), [tasks]);

    const onDelete = useCallback((task) => setTasks({
